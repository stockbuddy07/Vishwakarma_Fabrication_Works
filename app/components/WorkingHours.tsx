...
const WorkingHours = ({}: WorkingHoursProps) => {
      ...
      <div className="md:hidden">
        <section className={`py-8 px-4 ${darkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
          <div className="max-w-md mx-auto">
            <div className={`overflow-hidden rounded-3xl shadow-xl ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
              ...
              <div className="p-5 space-y-3">
                <div className={`p-4 rounded-2xl ${darkMode ? 'bg-gray-700' : 'bg-blue-50'}`}>
                  <p className={`text-[10px] font-bold opacity-60 uppercase mb-1 ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>{businessInfo.days}</p>
                  <p className={`text-lg font-black ${darkMode ? 'text-blue-300' : 'text-blue-600'}`}>{businessInfo.hours}</p>
                </div>
                
                {/* Status Section with dynamic color logic */}
                <div className={`p-4 rounded-2xl ${isOpen ? 'bg-green-50' : 'bg-red-50'}`}>
                  <div className={`flex items-center gap-2 ${isOpen ? 'text-green-500' : 'text-red-500'}`}>
                    <p className={`font-bold uppercase text-xs tracking-widest ${darkMode ? 'text-gray-300' : 'text-gray-400'}`}>Current Status</p>
      ...
      <div className="hidden md:block">
        <section className={`py-20 px-12 ${darkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
          <div className="max-w-6xl mx-auto">
            <div className={`flex rounded-[3rem] shadow-2xl overflow-hidden min-h-[500px] ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
              ...
              <div className="w-7/12 p-16 flex flex-col justify-center">
                <div className="grid grid-cols-1 gap-10 mb-12">
                  <div className="flex items-start gap-6">
                    <div className={`p-4 rounded-2xl ${darkMode ? 'bg-blue-900/30 text-blue-400' : 'bg-blue-50 text-blue-600'}`}><Clock size={32} /></div>
                    <div>
                      <p className={`font-bold uppercase text-xs tracking-widest mb-1 ${darkMode ? 'text-gray-400' : 'text-gray-400'}`}>{businessInfo.days}</p>
                      <p className={`text-4xl font-black ${darkMode ? 'text-gray-100' : 'text-gray-900'}`}>{businessInfo.hours}</p>
                    </div>
                  </div>
                  
                  <div className="flex items-start gap-6">
                    <div className={`p-4 rounded-2xl ${isOpen ? 'bg-green-50' : 'bg-red-50'} ${isOpen ? 'text-green-500' : 'text-red-500'}`}>
                    ...
                    <div>
                      <p className={`font-bold uppercase text-xs tracking-widest mb-1 ${darkMode ? 'text-gray-300' : 'text-gray-400'}`}>Current Status</p>
                ...
                <div className="flex gap-4">
                  <a 
                    href={businessInfo.phoneLink} 
                    aria-label="Call us at +91 98987 40255"
                    className="flex-1 flex items-center justify-center gap-3 py-5 min-h-[44px] rounded-2xl font-bold text-lg shadow-lg transition-transform hover:-translate-y-1 bg-gray-900 text-white"
                  >
